      ...
      >
        ...
        {items.length > 0 && (
          <div className="border-t border-gray-200/60 px-6 py-5 bg-white/50">
            ...
            {/* Annual Pass upsell - show when cart total > $60 and not a member */}
            {!isMember && totalCents > 6000 && (
              <Link
                href="/annual-pass"
                onClick={closeCart}
                className="mb-3 flex items-center gap-2 bg-gold/10 border border-gold/25 rounded-xl px-4 py-2.5 text-sm text-forest hover:bg-gold/15 transition-colors"
              >
                <svg className="w-4 h-4 flex-shrink-0 text-gold" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" aria-hidden="true">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M9.813 15.904L9 18.75l-.813-2.846a4.5 4.5 0 00-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 003.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 003.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 00-3.09 3.09zM18.259 8.715L18 9.75l-.259-1.035a3.375 3.375 0 00-2.455-2.456L14.25 6l1.036-.259a3.375 3.375 0 002.455-2.456L18 2.25l.259 1.035a3.375 3.375 0 002.455 2.456L21.75 6l-1.036.259a3.375 3.375 0 00-2.455 2.456z" />
                </svg>
                <span>
                  Your cart: <span className="font-semibold">{formatPrice(totalCents)}</span>. Get <span className="font-semibold">everything</span> for $99/year
                </span>
              </Link>
            )}
