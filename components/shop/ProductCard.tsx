...
export default function ProductCard({
  ...
  return (
    <Link href={`/shop/${slug}`} className="group block">
      <div className={`bg-white rounded-2xl shadow-sm group-hover:shadow-xl group-hover:shadow-forest/[0.08] transition-all duration-300 group-hover:-translate-y-2 group-hover:scale-[1.02] border border-gray-100/50 overflow-hidden border-l-[3px] ${categoryAccentColors[category] || 'border-l-[#588157]'}`}>
        {/* Product Image Area — Category-themed background */}
        <div className={`relative aspect-[4/3] ${categoryBgClasses[category] || 'card-bg-nature'} flex items-center justify-center p-6 overflow-hidden`}>
          ...
          {/* Bundle badge */}
          {isBundle && (
            <div className="absolute top-3 right-3 bg-gold text-white text-xs font-bold px-3 py-1.5 rounded-full shadow-md animate-pulse-glow">
              BEST VALUE
            </div>
          )}
          ...
          {/* Savings badge */}
          {!isBundle && savings > 0 && (
            <div className="absolute top-3 right-3 bg-gold text-white text-xs font-bold px-3 py-1.5 rounded-full shadow-sm shimmer-effect">
          ...
          {activityCount && (
            <div className="absolute bottom-3 right-3 bg-white/90 backdrop-blur-sm text-gray-700 text-xs font-semibold px-3 py-1.5 rounded-full shadow-sm">
              {activityCount} activities
            </div>
          )}

          {/* Category pill */}
          <div className="absolute bottom-3 left-3 bg-white/90 backdrop-blur-sm text-gray-700 text-xs font-medium px-3 py-1.5 rounded-full shadow-sm flex items-center gap-1.5">
