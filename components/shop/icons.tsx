...
// ─── UI Action Icons (Heroicons-style, strokeWidth=2) ───
// These live here (rather than duplicated inline) so the download/view/check
// buttons on the account pages and checkout flow share a single source.
...
export function CategoryIcon({ category, className, label }: { category: string; className?: string; label?: string }) {
  const Icon = categoryIconMap[category] || LeafIcon;
  return (
    <span role={label ? "img" : undefined} aria-label={label} aria-hidden={!label}>
